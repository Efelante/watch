...
cbuf_handle_t circular_buf_init(struct max30100_data *buffer, size_t size)
{
	assert(buffer && size);
...
void circular_buf_put(cbuf_handle_t me, struct max30100_data data)
{
	assert(me && me->buffer);
...
int circular_buf_try_put(cbuf_handle_t me, struct max30100_data data)
{
	int r = -1;
...
int circular_buf_get(cbuf_handle_t me, struct max30100_data *data)
{
...
int circular_buf_peek(cbuf_handle_t me, struct max30100_data *data, unsigned int look_ahead_counter)
{
	int r = -1;
	size_t pos;
