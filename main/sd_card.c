...
void sd_card_test(sdmmc_card_t *card, sdmmc_host_t *host)
{
    ...
    snprintf(data, EXAMPLE_MAX_CHAR_SIZE, "%s %i!\n", "Hello", (int)((card->cid).name));
    //snprintf(data, EXAMPLE_MAX_CHAR_SIZE, "%s %s!\n", "Hello", "CARD");
    ...
    snprintf(data, EXAMPLE_MAX_CHAR_SIZE, "%s %s!\n", "!!! Smart watch, Nihao, User and ", card->cid.name);
    //snprintf(data, EXAMPLE_MAX_CHAR_SIZE, "%s %s!\n", "!!! Smart watch, Nihao, User and ", "CARD");
...
void sd_card_initialize(sdmmc_card_t *card, sdmmc_host_t *host)
{
    ...
    sdmmc_host_t host_default = SDSPI_HOST_DEFAULT();
    *host = host_default;
...
#if CONFIG_EXAMPLE_SD_PWR_CTRL_LDO_INTERNAL_IO
	esp_err_t ret = sd_pwr_ctrl_del_on_chip_ldo(host->pwr_ctrl_handle);
	if (ret != ESP_OK) {
